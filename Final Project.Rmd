...
data_update <- data %>%
   ...
   filter(icu_flag == 0) %>%
   select(trans_losdays2, is30dayreadmit, cindex_new, evisit_new, ageyear, gender, race_new, religion, marital_new, insurance_new, respirationrate, bpdiastolic, o2sat, bpsystolic, temperature, heartrate, bmi, mews_new) %>%
   na.omit()
...
data_update <- data_update %>%
  select(-race_new, -religion,-gender)
```
